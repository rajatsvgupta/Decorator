...
def decorator_func(orig_func):
    logging.basicConfig(filename= f'{orig_func.__name__}_e.log', level=logging.INFO)
    @wraps(orig_func)
    def wrapper(*args,**kwargs):
...
#check wraps added in one but corey told in everyone but first decorator function(here decorator func) shd have
